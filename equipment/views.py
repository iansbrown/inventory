...
def scheduling_dashboard_view(request):
    ...
    for course in courses:
        for offering in course.offerings.all():
            for week in offering.schedule_weeks.all():
                ...
                all_week_numbers.add(week_num)
    
                meetings = list(week.meetings.all())
    
                schedule_grid[week_num][course] = meetings
    
    
    # Sort properly
    sorted_weeks = sorted(all_week_numbers)
    ...
    for eq_type in EquipmentType.objects.all():
        ...
        items = EquipmentItem.objects.filter(
            ...
            status=EquipmentItem.STATUS_AVAILABLE  # ✅ optional but recommended
        )
    
        # BULK: sum quantities
        bulk_total = items.filter(
            tracking_level=EquipmentItem.TRACKING_BULK
        ).aggregate(
            total=Sum("quantity")
        )["total"] or 0
    
        # INDIVIDUAL: count rows
        individual_total = items.exclude(
            tracking_level=EquipmentItem.TRACKING_BULK
        ).count()
